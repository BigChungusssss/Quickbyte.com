    ...
    async function loadLeaders() {
      ...
      document.getElementById('leaders').innerHTML = leaders.map(l => `
        <div class="leader" data-id="${l.id}">
          ...
          <button data-action="remove-leader" data-id="${l.id}" style="margin-top:8px;color:#c0392b;">Remove leader entirely</button>
        </div>
      `).join('');
    ...
    (async () => {
      ...
      if (!check.ok) {
        document.body.innerHTML = '<p style="font-family:sans-serif;color:#c0392b;">Not authorized.</p>';
        return;
      }
